...
<div class="container card p-4">
    ...
    <form action="{{ isset($rule) ? route('inventory-rules.update', $rule->id) : route('inventory-rules.store') }}" method="POST">
        @csrf
        @if (isset($rule))
            @method('PUT')
        @endif
        ...
        <div class="mb-3">
            ...
            <select name="dakar_role_id" class="form-select">
                ...
                @foreach ($roles as $role)
                    <option value="{{ $role->id }}" {{ isset($rule) && $rule->dakar_role_id == $role->id ? 'selected' : '' }}>
                        {{ $role->role_name }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="mb-3">
            ...
            <div id="dep-container">
                @if (isset($rule) && $rule->department->count() > 0)
                    @foreach ($rule->department as $selecteddepartment)
                        <div class="d-flex align-items-center mb-2 dep-group">
                            <select name="department_id[]" class="form-select">
                                @foreach ($departments as $department)
                                    <option value="{{ $department->id }}" {{ $selecteddepartment->id == $department->id ? 'selected' : '' }}>
                                        {{ $department->department_name }}
                                    </option>
                                @endforeach
                            </select>
                ...
                @else
                    <div class="d-flex align-items-center mb-2 item-group">
                        ...
                        <button type="button" class="btn btn-outline-danger btn-sm ms-2 remove-dep"><i class="ti ti-trash"></i></button>
                    </div>
                @endif
            </div>
            <button type="button" id="add-dep" class="btn btn-success mt-2">Tambah Department</button>
        </div>
        ...
        <div class="mb-3">
            ...
            <div id="items-container">
                @if (isset($rule) && $rule->items->count() > 0)
                    @foreach ($rule->items as $selectedItem)
                        <div class="d-flex align-items-center mb-2 item-group">
                            <select name="items[]" class="form-select">
                                @foreach ($items as $item)
                                    <option value="{{ $item->id }}" {{ $selectedItem->id == $item->id ? 'selected' : '' }}>
                                        {{ $item->item_name }}
                                    </option>
                                @endforeach
                            </select>
...
<script>
    $(document).ready(function() {
        ...
        $('#add-dep').click(function() {
            ...
            $('#dep-container').append(newSelect);
        });
