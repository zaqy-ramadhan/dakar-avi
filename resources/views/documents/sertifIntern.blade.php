...
<body>
  <div class="certificate" role="main" aria-label="Certificate of Completion">
      ...
      />
      <h1>CERTIFICATE OF {{ Str::upper($type) }}</h1>
      <h2>PT. ASTRA VISTEON INDONESIA</h2>
      <p>PRESENTED TO</p>
      <h2>{{ Str::upper($kontrak->user->fullname) }}</h2>
      <p class="small-text">
        HAS COMPLETED THE {{ Str::upper($type)  }} PROJECT AT<br />
        PT. ASTRA VISTEON INDONESIA<br />
        PERIOD
      </p>

      <div class="date">
          <h3>{{ $kontrak->start_date->format('F jS Y') . ' — ' . $kontrak->end_date->format('F jS Y') }}</h3>
      </div>
