    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('actions', function ($job) {
                ...
                //     : '';
                $skhkButton = $job->user_dakar_role !== 'internship' && $job->employment_status == false ? '<a title="SKSMK" href="' . route("user.skhk-pdf", $job->id) . '" class="btn btn-sm btn-outline-primary m-1"><i class="ti ti-hourglass-off fs-6"></i> SKSMK</a>' : '';
                ...
                $wageButton = '';
                if ($showWageButton) {
                    $wageButton = '<a title="Gaji & Tunjangan" href="' . route("job.wage.allowance", $job->id) . '" class="btn btn-sm btn-outline-primary m-1"><i class="ti ti-wallet fs-6"></i> Gaji & Tunjangan</a>';
                }
